...
public class NldasNOAHDownloadTask implements Runnable {
    DataDate startDate;
    static final String downloadedCache = Paths.get("").toAbsolutePath() + "\\Nldas\\downloaded.xml.gz";
    static final String onlineAvaliableCache = Paths.get("").toAbsolutePath() + "Nldas\\avaliable.xml.gz";
    List<DataDate> available;
    List<DataDate> finished;
    DownloadMetaData mode;

    public NldasNOAHDownloadTask(DataDate dataDate, DownloadMetaData metaData) {
        startDate = dataDate;
        mode=metaData;
    }
    ...
    public void run(){
        ...
        for (DataDate item : download) {
            try {
                ...
                new NldasNOAHDownloader(item, getoutFile(item),mode).download();
            } catch (Exception e) {
    ...
    private NldasCache getOnlineDataList() throws IOException {
        ...
        try {
            updatedCache = new NldasCache(DataDate.today(), startDate, NldasNOAHDownloader.listDates(startDate));
