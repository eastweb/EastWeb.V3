...
public class Indices extends Process<Void>{
    private ArrayList<IndicesWorker> workers;

    public Indices(ProjectInfoFile projectInfoFile, ProjectInfoPlugin pluginInfo, PluginMetaData pluginMetaData, Scheduler scheduler, ThreadState state,
            ProcessName processName, String inputTableName, ExecutorService executor)
    {
        ...
        workers = new ArrayList<IndicesWorker>(0);
    }

    @Override
    public Void call() throws Exception {
        IndicesWorker worker;
        ...
        if(cachedFiles.size() > 0)
        {
            ...
            {
                worker = new IndicesWorker(this, projectInfoFile, pluginInfo, pluginMetaData, cachedFiles);
                workers.add(worker);
        ...
        scheduler.NotifyUI(new GeneralUIEventObject(this, "Indices Finished", 100));
        return null;
    }
